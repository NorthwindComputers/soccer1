...
$(document).ready(function() {
	$('button#plzLogin.affBtn').click(function() {
		window.location.href = "myaccount.php?prodid=<?php echo $_GET['id']; ?>";
	});
});
*/
</script>
...
<?php
		...
		while ($row_moreimg = mysql_fetch_array($result_moreimg)) {
			...
			$firstimg = '<img id="mainMore" src="cpadmin/images/productView/'.$last[0].'" style="height: 280px; padding: 10px;" />';
			$imglist .= '<td><img src="cpadmin/images/productView/'.$row_moreimg["Image"].'" class="thumbs" /></td>';	
		}
...
?>
    		<div class="detailed_L">
      		<meta property="og:image" content="cpadmin/images/productImages/<?=$Image;?>" />
    		<img id="mainimg" style="width: 228px;" src="cpadmin/images/productImages/<?=$Image;?>" alt="<?=$row_mainimg['AltText'];?>" />
<?php 
			if ($prodid != 'VIP') {
...
?>
            ...
            <?php if ($isSpecial == "True") {
				...
				// Pricing chart
				echo '<table id="member_table" cellpadding="0" cellspacing="0"><tr><td style="text-align: center;"><div class="discount_pricing"><h4>VIP Member Discount Pricing</h4><div class="clear"></div><ul>';
        		...
        		echo '<li style="background:none;">&nbsp;</li><li class="non_members">MSRP/Value</li><li class="non_members">Non Member</li><li class="vip_C2">' . $row_pricing["Option1"] . '</li><li class="vip_C3">' . $row_pricing["Option2"] . '</li><li class="vip_C4">' . $row_pricing["Option3"] . '</li><li class="yellow" style="margin-right:0px;">' . $row_pricing["Option4"] . '</li>';
                ...
                while ($row_pricing = mysql_fetch_array($result_pricing)) {
					if ($row_pricing["Gender"] == '') {
						$genclass = ' style="background-color: #ffffff;"';
					} else {
						$genclass = 'class="red"';
					}
        			echo '<li ' . $genclass . '>' . $row_pricing["Gender"] . '</li><li class="non_members"><s>$' . number_format($row_pricing["MSRP"],2) . '</s></li><li class="non_members">$' . number_format($row_pricing["NonMember"],2) .'</li><li class="vip_C2">$' . number_format($row_pricing["Option1Price"],2) . '</li><li class="vip_C3">$' . number_format($row_pricing["Option2Price"],2) . '</li><li class="vip_C4">$' . number_format($row_pricing["Option3Price"],2) . '</li><li class="yellow" style="margin-right:0px;">$' . number_format($row_pricing["Option4Price"],2) . '</li>';
        		}
        		echo '</ul></div></td></tr></table>';
			}
            } 
            if ($isvip == 'no' && $isSpecial != "True") {
                echo '<div class="clear"></div><div class="VIP_members"><img src="images/S_soccer_card.png" alt="VIP Membership - JOIN TODAY!"><h2>VIP MEMBERS SAVE EVEN MORE</h2><h3>Not a VIP member? <a href="details.php?id=VIP">Join Today</a></h3></div>';
            }
            ?>
            <div class="mobileDesc"><?=$ShortDescription;?></div>
    		</div><!-- END MOBILE -->
...
?>
        ...
        <?php
                if ($isSpecial != "True") {
        ?>
                	<table id="member_table" cellpadding="0" cellspacing="0" style="width: 100%;">
                    <tr>
                    	<td style="text-align: center;">
		...
		<?php
			if ($prodid != 'VIP') {
				if ($ProductType == "Bundle") {
					if ($ProductType == 'Bundle') {
						$imgPath = 'cpadmin/images/productView/';
					} else {
						$imgPath = 'cpadmin/images/productImages/';
					}
 ...
 <?php
	if ($ProductType == "Bundle") {
		$rightbarheight = 'style="height: 690px !important;"';
	} else {
		$rightbarheight = '';
	}
?>
